...
public:
    ...
    [[nodiscard]] virtual std::string getSrc() const = 0;

    /// Returns path to the destination (copy-to) file, path is expected to be absolute
    [[nodiscard]] virtual std::string getDst() const = 0;
    ...
    [[nodiscard]] virtual std::string getSharedObjName() const = 0;

    /// Returns bool if tcpip tool instance should serve as server
    /// Otherwise false
    [[nodiscard]] virtual bool getIsServer() const = 0;

    [[nodiscard]] virtual std::vector<std::string> getFilesToSend() const = 0;

    [[nodiscard]] virtual std::string getSettingsPath() const = 0;
